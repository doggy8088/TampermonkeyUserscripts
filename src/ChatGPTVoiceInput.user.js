...
(async function () {
    ...
    speakerButtonElement.title = `${EnableSpeechSynthesis() ? '關閉' : '開啟'}語音辨識功能 (${isMac() ? 'command+option+m' : 'alt+m'})`;
    speakerButtonElement.innerHTML = EnableSpeechSynthesis() ? svgSpeakerOn : svgSpeakerOff;
    ...
    class VoiceInputHelper {
        ...
        Start() {
            ...
            this.microphoneButton.innerHTML = svgMicOn;
            this.microphoneButton.title = `關閉語音辨識功能 (${isMac() ? 'command+option+s' : 'alt+s'})`;
        }
        ...
        Stop() {
            ...
            this.microphoneButton.innerHTML = svgMicOff;
            this.microphoneButton.title = `開啟語音辨識功能 (${isMac() ? 'command+option+s' : 'alt+s'})`;
        }
    ...
    const SpeakText = (videoInputHelper, text) => {
        return new Observable(subscriber => {
            ...
            utterance.onend = (evt) => {
                ...
                if (!ChatGPTRunningStatus && !speechSynthesis.pending && isMicrophoneOn) {

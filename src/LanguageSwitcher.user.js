    ...
    document.addEventListener('keydown', (ev) => {
        ...
        if (isAltS && !inExcludedTargets) {
            ...
            (function () {
                ...
                if (ln.indexOf('//angular.dev/') >= 0 || ln === 'https://angular.dev/' || ln === 'http://angular.dev/') {
                    void (location.href = ln.replace(/angular\.dev/i, 'dev.angular.tw'));
                }
                if (ln.indexOf('//dev.angular.tw/') >= 0 || ln === 'https://dev.angular.tw/' || ln === 'http://dev.angular.tw/') {
                    void (location.href = ln.replace(/dev\.angular\.tw/i, 'angular.dev'));
                }
                ...
                if (ln.indexOf('//code.visualstudio.com/') >= 0) {
                    void (location.host = 'vscode.dev.org.tw');
                }
                if (ln.indexOf('//vscode.dev.org.tw/') >= 0) {
                    void (location.host = 'code.visualstudio.com');
                }
                ...
                // 通用的中文路徑轉英文路徑邏輯 (fallback)
                if (pn.search(/^\/zh-(tw|cn)(\/|$)/i) >= 0) {
                    void (location.pathname = pn.replace(/^\/zh-(tw|cn)(\/)?/i, '/en$2'));
                }
            }());
        }
    });
